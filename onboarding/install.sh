...
generate_configuration_files() {
    ...
    if command -v curl >/dev/null 2>&1; then
        print_info "Downloading nginx.conf for browser proxy..."
        curl -sfL "https://github.com/ClawFarm-by-INFAC/ClawFarm/raw/main/docker/browser-proxy/nginx.conf" -o "$DEPLOY_DIR/nginx.conf" || {
            print_warning "Failed to download nginx.conf, browser proxy may not work"
        }
    fi
