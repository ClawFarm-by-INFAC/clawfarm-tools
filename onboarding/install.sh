...
generate_configuration_files() {
    ...
    local nginx_conf_source="$script_dir/nginx.conf"
    ...
    if [[ -f "$nginx_conf_source" ]]; then
        ...
        print_warning "Browser proxy may not work correctly"
        # Create a minimal nginx.conf as fallback
        cat > "$DEPLOY_DIR/nginx.conf" <<'EOF'
# Minimal nginx.conf fallback
events {
...
http {
    server {
        ...
        server_name _;
        location / {
            return 503 '{"error":"nginx.conf not properly configured"}';
